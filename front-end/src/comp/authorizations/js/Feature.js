...
function Feature() {
  ...
  const handleGetData = (e) => {
    axios
      .get(`${process.env.REACT_APP_HOSTNAME}/api/feature-full-data/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleEdit = (e) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_HOSTNAME}/api/feature/${e.target.id.dataset.id}/`,
        {
      ...
      .catch((error) => {
        if (error.response.status === 400) {
          swal("Error!", "Feature Already Added!", "error");
        } else {
              ...
              onClick={() => {
                swal({
                  ...
                  if (willDelete) {
                    axios
                      .delete(
                        `${process.env.REACT_APP_HOSTNAME}/api/feature/${row.id}/`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                          },
  ...
  const handleFilter = (e) => {
    const newData = data.filter((row) => {
      return row.feature.toLowerCase().includes(e.target.value.toLowerCase());
    });
    setRecord(newData);
  };
