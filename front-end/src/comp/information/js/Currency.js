...
function Currency() {
  ...
  const handleGetData = (e) => {
    axios
      .get(`${process.env.REACT_APP_HOSTNAME}/api/currency-full-data/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleEdit = (e) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_HOSTNAME}/api/currency/${e.target.id.dataset.id}/`,
        {
      ...
      .catch((error) => {
        if (error.response.status === 400) {
          swal("Error!", "Currency Already Added!", "error");
        } else {
              ...
              onClick={() => {
                swal({
                  ...
                  if (willDelete) {
                    axios
                      .delete(
                        `${process.env.REACT_APP_HOSTNAME}/api/currency/${row.id}/`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                          },
  ...
  const handleFilter = (e) => {
    const newData = data.filter((row) => {
      return row.currency.toLowerCase().includes(e.target.value.toLowerCase());
    });
    setRecord(newData);
  };
    ...
    <>
      ...
      <div className="bg-sec min-vh-100 py-5 ">
        <div className="container-fluid px-4">
          <h2 className="main-title">Currency</h2>
