...
function Language() {
  ...
  const handleEdit = (e) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_HOSTNAME}/api/language/${e.target.id.dataset.id}/`,
        {
      ...
      .catch((error) => {
        if (error.response.status === 400) {
          swal("Error!", "Language Already Added!", "error");
        } else {
              ...
              onClick={() => {
                swal({
                  ...
                  if (willDelete) {
                    axios
                      .delete(
                        `${process.env.REACT_APP_HOSTNAME}/api/language/${row.id}/`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                          },
  ...
  const handleFilter = (e) => {
    const newData = data.filter((row) => {
      return row.language.toLowerCase().includes(e.target.value.toLowerCase());
    });
    setRecord(newData);
  };
