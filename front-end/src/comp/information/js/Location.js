...
function Location() {
  ...
  const handleEdit = (e) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_HOSTNAME}/api/location/${e.target.id.dataset.id}/`,
        {
      ...
      .catch((error) => {
        if (error.response.status === 400) {
          swal("Error!", "Location Already Added!", "error");
        } else {
              ...
              onClick={() => {
                swal({
                  ...
                  if (willDelete) {
                    axios
                      .delete(
                        `${process.env.REACT_APP_HOSTNAME}/api/location/${row.id}/`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                          },
  ...
  const handleFilter = (e) => {
    const newData = data.filter((row) => {
      return row.longitude.includes(e.target.value);
    });
    setRecord(newData);
  };
