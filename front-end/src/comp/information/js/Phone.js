...
function Phone() {
  ...
  const handleEdit = (e) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_HOSTNAME}/api/phone/${e.target.id.dataset.id}/`,
        {
      ...
      .catch((error) => {
        if (error.response.status === 400) {
          swal("Error!", "Phone Already Added!", "error");
        } else {
              ...
              onClick={() => {
                swal({
                  ...
                  if (willDelete) {
                    axios
                      .delete(
                        `${process.env.REACT_APP_HOSTNAME}/api/phone/${row.id}/`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                          },
    ...
    <>
      ...
      <div className="bg-sec min-vh-100 py-5 ">
        <div className="container-fluid px-4">
          <h2 className="main-title">Phone</h2>
